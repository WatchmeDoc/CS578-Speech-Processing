...
for j = 1:length(coeffs)
    ...
    for k = 1:length(myFiles)
        ...
        Ethres = energy_coeff * mean(energy)/2;
        ZCRthres = zcr_coeff * ((3/2)*mean(ZCr) - 0.3*std(ZCr));
        ...
        xlabel('Time (s)');
        ylim([-1.5 1.5]);
        title(['Energy coeff = ', num2str(energy_coeff), ' ZCr coeff = ', num2str(zcr_coeff)], 'Interpreter', 'none');
